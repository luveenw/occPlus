runMCMCOccPlus <- function(data,
  ...
  # pcr per marker
  {
    data_K <- data_info %>%
      ...
      dplyr::summarise(
        K = n(),
        dplyr::across(contains("Species"), function(x) {
          sum(x > 0)
        })
      ) %>%
      dplyr::ungroup() # %>%
  ...
  for (chain in 1:nchain) {
    ...
    for (iter in 1:(niter + nburn)) {
      ...
      {
        list_betapsiLL <- sample_psivars(
          z, X_psi, beta_psi, X_ord, beta_ord,
      ...
      # sample w
      w <- sample_w_cpp(
        logy1, mu0, sigma0, mu1, sigma1, theta, theta0, p, q,
      ...
      # sample theta
      beta_theta <- sample_betatheta(
        w, beta_theta, idx_z, X_theta,
